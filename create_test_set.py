...
sp500_clean = pd.read_csv("Data/train_set.csv", index_col="Timestamp", parse_dates=True)
...
test_set = sp500_clean.iloc[N_clean-TEST_SET_LENGTH:, :]
test_set.to_csv("Data/test_set.csv")

# Shorten each file by removing last 30 points
#### WARNING:
# - this action is not idempotent. you'll just keep shortenining and lose data, so dont run it to many times
sp500_clean_shortened = sp500_clean.iloc[:N_clean-(TEST_SET_LENGTH-30), :]
sp500_raw_shortened = sp500_raw.iloc[:N_raw-(TEST_SET_LENGTH-30), :]
spx_raw_shortened = spx_raw.iloc[:N_spx-(TEST_SET_LENGTH-30), :]

# Save the shortened files (overwriting originals)
sp500_clean_shortened.to_csv("Data/train_set.csv")
...
print(f"Shortened train_set.csv from {N_clean} to {sp500_clean_shortened.shape[0]} rows")
